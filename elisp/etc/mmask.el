...
(defun mmask-regist-name (modesym &rest values)
  (let* ((regists (gethash modesym mmask--path-regexp-hash))
         (values (cl-loop for v in values
                          if (and (stringp v)
                                  (not (string= v "")))
                          collect v)))
...
(defun mmask-get-mode-matched (filename_or_path)
  (let* ((case-fold-search nil))
    (or (cl-loop for mode being the hash-keys in mmask--extension-hash
                 for re = (mmask-get-regexp-string mode)
                 if (string-match re filename_or_path)
                 return mode)
        (cl-loop for mode being the hash-keys in mmask--path-regexp-hash
                 for re = (mmask-get-regexp-string mode)
                 if (string-match re filename_or_path)
                 return mode))))
...
(defun mmask--get-ignore-case-strings (s)
  ...
  ;;                     (list ""))))
  ;;   (cl-loop for c in (or (when (string-match "^[a-zA-Z]$" c)
  ;;                        (list (downcase c) (upcase c)))
  ;;                      (list c))
  ;;         append (cl-loop for s in strings
  ;;                      collect (concat c s)))))
  ...
  (when mmask--update-auto-mode-alist-p
    (let* ((re (mmask-get-regexp-string modesym))
           (newal (cond ((rassq modesym mmask--default-auto-mode-alist)
                         auto-mode-alist)
                        (t
                         (cl-loop for e in auto-mode-alist
                                  for m = (cdr e)
                                  if (not (eq m modesym))
                                  collect e)))))
